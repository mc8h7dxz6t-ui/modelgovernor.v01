"""Finance Governor sidecar — OIDC RBAC for privileged internal APIs."""
...
def _validate_oidc_jwt(token: str) -> AuthContext:
    ...
    try:
        ...
        logger.info("fg sidecar oidc rejected: %s", exc)
...
def _validate_internal_token(x_internal_token: str | None) -> AuthContext | None:
    settings = get_settings()
    allowed = {t.strip() for t in settings.fg_internal_tokens.split(",") if t.strip()}
    if x_internal_token and x_internal_token in allowed:
        roles = {"financial-admin", "viewer"} if settings.oidc_internal_token_is_admin else {"viewer"}
        return AuthContext(
...
async def require_financial_admin(
    ...
    if not ctx.is_financial_admin():
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="financial admin role required")
    return ctx
