"""Finance Governor gateway — OIDC + internal token auth."""
...
class GatewayAuthContext:
    ...
    def can_commit(self) -> bool:
        ...
        if self.method == "internal_token":
            return True
        allowed = settings.oidc_commit_roles_list()
        if not allowed:
            return True
        return bool(self.roles.intersection({r.strip().lower() for r in allowed}))
...
def _validate_oidc_jwt(token: str) -> GatewayAuthContext:
    ...
    if not settings.oidc_issuer_url:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="OIDC issuer not configured")

    try:
        from jwt import InvalidTokenError, decode
    except ImportError as exc:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="PyJWT required") from exc
    ...
    try:
        ...
        logger.info("fg gateway oidc rejected: %s", exc)
    ...
    return GatewayAuthContext(method="oidc", subject=subject, roles=frozenset(_extract_roles(claims)))


def _validate_internal_token(x_internal_token: str | None) -> GatewayAuthContext | None:
    settings = get_settings()
    if x_internal_token and x_internal_token == settings.fg_internal_token:
        return GatewayAuthContext(method="internal_token", subject="internal-token", roles=frozenset({"fg-commit"}))
    return None


async def require_governed_auth(
    ...
    if settings.oidc_enabled:
        if bearer:
            ...
            if not ctx.can_commit():
                raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="insufficient OIDC role")
            return ctx
        if settings.oidc_allow_internal_token_fallback:
            token_ctx = _validate_internal_token(x_internal_token)
            if token_ctx is not None:
                return token_ctx
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="missing OIDC bearer token")
    ...
    if bearer:
        ...
        if ctx.can_commit():
            return ctx
    raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="missing gateway credentials")
