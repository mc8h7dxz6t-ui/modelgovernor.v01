"""Incremental decision chain verification — O(delta) checkpoint semantics."""
...
CHECKPOINT_DDL = """
CREATE TABLE decision_chain_verify_checkpoints (
...
def test_incremental_verify_uses_checkpoint_on_unchanged_head(spine_db):
    ...
    with factory() as session:
        first = verify_decision_chain(session, incremental=True)
        assert first.valid is True
        assert first.incremental is False

    with factory() as session:
        second = verify_decision_chain(session, incremental=True)
...
def test_incremental_verify_tail_after_new_events(spine_db):
    ...
    factory = sessionmaker(bind=spine_db)
    with factory() as session:
        baseline = verify_decision_chain(session, incremental=True)
        assert baseline.valid is True

    _crystallize(client, "inc-tail")
    with factory() as session:
        updated = verify_decision_chain(session, incremental=True)
