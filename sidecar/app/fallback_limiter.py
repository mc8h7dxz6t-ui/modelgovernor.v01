...
Prevents split-brain guardrail bypass from stampeding Postgres during Redis outages.
Uses a thread-safe token bucket (global) plus per-user rate and in-flight caps.
"""
    ...
    def check_reserve(
        ...
        *,
        ...
        with self._lock:
            ...
            prior = self._rate_windows.get(user_id)
            count = 1 if prior is None or prior[0] != window else prior[1] + 1
            self._rate_windows[user_id] = (window, count)
            if count > settings.fallback_rate_limit_per_minute:
                get_counters().increment("local_fallback_rate_limit_total")
                raise RateLimitExceeded(f"local fallback rate limit for user {user_id}")

            trace_ops = self._trace_depth[trace_id]
            trace_ops.add(idempotency_key)
            if len(trace_ops) > settings.fallback_max_trace_depth:
                get_counters().increment("local_fallback_trace_depth_total")
                raise TraceDepthExceeded(f"local fallback trace depth for {trace_id}")

            inflight = self._inflight[user_id] + 1
            if inflight > settings.fallback_max_user_inflight:
                get_counters().increment("local_fallback_inflight_total")
                raise InflightLimitExceeded(f"local fallback in-flight limit for user {user_id}")

            self._inflight[user_id] = inflight
            self._tokens -= 1.0
            get_counters().increment("local_fallback_reserve_total")
