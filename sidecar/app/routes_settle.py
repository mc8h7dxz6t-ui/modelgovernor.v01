...
from app.metrics import settle_amount_total, settle_requests_total
...
def settle(request: SettleRequest) -> SettleResponse:
    try:
        ...
        settle_requests_total.labels(outcome="rejected").inc()
    ...
    try:
        with get_db_session() as session:
            ...
            if ledger_row["status"] == "SETTLED":
                ...
                settle_requests_total.labels(outcome="replayed").inc()
            ...
            refund_amount = quantize_money(max(reserved_amount - actual_amount, Decimal("0")))
        ...
        settle_requests_total.labels(outcome="error").inc()
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="governance service temporarily unavailable",
        ) from exc

    settle_requests_total.labels(outcome="settled").inc()
    settle_amount_total.inc(float(actual_amount))
