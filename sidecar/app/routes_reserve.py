...
from app.metrics import reserve_amount_total, reserve_requests_total
...
def reserve(request: ReserveRequest) -> ReserveResponse:
    try:
        ...
        reserve_requests_total.labels(outcome="rejected").inc()
    ...
    try:
        with get_db_session() as session:
            ...
            if existing is not None:
                ...
                reserve_requests_total.labels(outcome="replayed").inc()
            ...
            model_policy = session.execute(
                text(
                    """
                    SELECT enabled, max_cost_per_request, max_cost_per_trace
            ...
            # Per-trace spend cap enforcement.
            max_cost_per_trace = model_policy.get("max_cost_per_trace")
            if max_cost_per_trace is not None:
                trace_spend_row = session.execute(
                    text(
                        """
                        SELECT COALESCE(SUM(
                            CASE WHEN status = 'RESERVED' THEN reserved_amount
                    ...
                    {"user_id": request.user_id, "trace_id": request.trace_id},
                ).mappings().first()

                trace_spend = quantize_money(trace_spend_row["trace_spend"])
                max_trace_spend = quantize_money(max_cost_per_trace)

                if trace_spend + reserve_amount > max_trace_spend:
                    reserve_requests_total.labels(outcome="rejected").inc()
                    raise HTTPException(
                        status_code=status.HTTP_402_PAYMENT_REQUIRED,
                        detail="per-trace spend cap exceeded",
                    )
        ...
        reserve_requests_total.labels(outcome="error").inc()
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="governance service temporarily unavailable",
        ) from exc

    reserve_requests_total.labels(outcome="accepted").inc()
    reserve_amount_total.inc(float(reserve_amount))
