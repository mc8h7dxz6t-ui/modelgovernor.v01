...
- Spend report: `GET /admin/spend-report`
- Wallet summary: `GET /admin/wallet-summary/{wallet_id}`
- Reconciliation summary: `GET /admin/reconciliation-summary`
- Stranded operations: `GET /admin/stranded-operations`
...
4. Unlock the wallet only after review via `POST /admin/unlock-wallet`.

### Stranded operations growing

Symptoms:
- `reconciler_stranded_total` rising
- `GET /admin/stranded-operations` returns accumulating rows

Response:
1. Verify provider outcome for each stranded operation.
2. Apply deterministic correction with `POST /admin/correct-operation` when the
