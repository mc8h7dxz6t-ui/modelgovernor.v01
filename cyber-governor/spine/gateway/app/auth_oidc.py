"""Gateway-edge OIDC JWT validation before governed commit."""
...
class GatewayAuthContext:
    ...
    def can_commit(self) -> bool:
        ...
        if self.method == "internal_token":
            return True
        dispatch_roles = settings.oidc_commit_roles_list()
...
def _validate_internal_token(x_internal_token: str | None) -> GatewayAuthContext | None:
    settings = get_settings()
    if x_internal_token and x_internal_token == settings.cg_internal_token:
        return GatewayAuthContext(
            method="internal_token",
            subject="internal-token",
            roles=frozenset({"commit"}),
        )
    return None


async def require_commit_auth(
        ...
        if bearer:
            ...
            if not ctx.can_commit():
                raise HTTPException(
                    ...
                    detail="insufficient OIDC role for governed commit",
                )
            return ctx
