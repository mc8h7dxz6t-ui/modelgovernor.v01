...
class SealModuleSpec:
    """Required seal/verify surface for a governor spine."""
...
def seal_module_path(domain: GovernorDomain, repo_root: Path | None = None) -> Path:
    root = repo_root or REPO_ROOT
    return root / SEAL_REGISTRY[domain].rel_path
...
def conformance_failures(repo_root: Path | None = None) -> list[str]:
    """Return human-readable K1 conformance gaps (empty list = pass)."""
    root = repo_root or REPO_ROOT
    failures: list[str] = []
