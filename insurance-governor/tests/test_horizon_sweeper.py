    ...
    session.execute(
        text(
            ...
            INSERT INTO governance_crystals (
                ...
                :cid, 'claim_gate', :op, :tier, :facets, 'fp', 'ch', :horizon
            )
            """
        ),
        {
            ...
            "op": operation_id,
            "tier": risk_tier,
            "facets": json.dumps({"claim_id": operation_id, "payout_amount": "1.00"}),
    ...
    session.execute(
        text(
            ...
            INSERT INTO claim_escrow_ledger (
                ...
                :op, :cid, 'carrier-default', 'claim_gate', 10.0, 'CRYSTALLIZED', :horizon
            )
            """
        ),
...
def test_high_risk_horizon_strands(spine_db):
    ...
    with factory() as session:
        ...
        status = session.execute(
            text("SELECT status FROM claim_escrow_ledger WHERE crystal_id = 'c-strand'")
        ).scalar_one()
...
def test_in_flight_horizon_strands(spine_db):
    ...
    with factory() as session:
        ...
        session.execute(
            text("UPDATE claim_escrow_ledger SET status = 'IN_FLIGHT' WHERE crystal_id = 'c-inflight'")
        )
        session.commit()
