...
def test_surprise_commit_blocked_unknown_crystal(client):
    ...
    r = client.post(
        ...
        json={
            ...
            "facets": {"claim_id": "x", "payout_amount": "1.00"},
            "committed_reserve": "0",
        },
    )
...
def test_fingerprint_mismatch_increments_counter(client):
    facets = {"claim_id": "inv-fp-1", "payout_amount": "25.00"}
    cry = client.post(
        "/crystallize",
        headers=HEADERS,
        json={
    ...
    bad = client.post(
        ...
        json={
            ...
            "facets": {"claim_id": "inv-fp-1", "payout_amount": "99.00"},
            "committed_reserve": "0",
        },
    )
...
def test_high_risk_expired_horizon_strands_on_commit(client, spine_db):
    facets = {"claim_id": "inv-horizon-1", "payout_amount": "10.00"}
    cry = client.post(
        "/crystallize",
        headers=HEADERS,
        json={
    ...
    r = client.post(
        "/commit",
        headers=HEADERS,
        json={"crystal_id": crystal_id, "facets": facets, "committed_reserve": "0"},
    )
    assert r.status_code == 409
