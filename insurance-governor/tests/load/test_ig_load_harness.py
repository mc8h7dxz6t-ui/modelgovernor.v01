"""IG crystallize/commit load harness — correctness under concurrency."""
...
def _crystallize_commit(worker: int, op: int) -> tuple[float, str]:
    ...
    facets = {"claim_id": op_id, "payout_amount": "10.00"}
    ...
    try:
        ...
        commit_operation(session, crystal_id=c.crystal_id, facets=facets, committed_reserve=Decimal("0"))
        status = "ok"
