"""Toxiproxy-backed chaos tests for Insurance Governor claim ops."""
...
def _postgres_url() -> str:
    ...
    if not url:
        pytest.skip(
            "POSTGRES_TEST_URL not set — start insurance-governor/docker-compose.chaos.yml and export "
            "POSTGRES_TEST_URL=postgresql+psycopg://postgres:postgres@localhost:5436/ig_chaos"
        )
    return url
        ...
        if recreate:
            ...
            requests.post(
                ...
                json={
                    "name": PROXY_NAME,
                    "listen": "0.0.0.0:5436",
                    "upstream": "ig-postgres-chaos:5432",
                    "enabled": True,
                },
                timeout=5,
            ).raise_for_status()
...
def test_claim_ops_survives_toxiproxy_latency(chaos_engine) -> None:
    ...
    with factory() as session:
        crystallize_operation(
            ...
            settings,
            platform="claim_gate",
            operation_id="chaos-claim-1",
            account_id="carrier-default",
            risk_tier="high",
            facets={"claim_id": "chaos-claim-1", "payout_amount": "100.00"},
            policy_id="claim-high-us",
            reserved_reserve=Decimal("50"),
        )
    ...
    with Session(chaos_engine) as session:
        balance = session.execute(
            text(
                """
                SELECT balance FROM reserve_ledgers
...
def test_claim_ops_toxiproxy_timeout_recovers_on_reset(chaos_engine) -> None:
    from app.commit_ledger import crystallize_operation
    ...
    with pytest.raises(Exception):
        with factory() as session:
            crystallize_operation(
                ...
                settings,
                platform="claim_gate",
                operation_id="chaos-claim-2",
                account_id="carrier-default",
                risk_tier="high",
                facets={"claim_id": "chaos-claim-2", "payout_amount": "25.00"},
                policy_id="claim-high-us",
                reserved_reserve=Decimal("10"),
            )
    ...
    with factory() as session:
        crystallize_operation(
            ...
            settings,
            platform="claim_gate",
            operation_id="chaos-claim-3",
            account_id="carrier-default",
            risk_tier="high",
            facets={"claim_id": "chaos-claim-3", "payout_amount": "25.00"},
            policy_id="claim-high-us",
            reserved_reserve=Decimal("10"),
        )
