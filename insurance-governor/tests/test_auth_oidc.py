"""OIDC RBAC tests for Insurance Governor sidecar."""
...
def _configure(spine_db, monkeypatch, *, oidc_enabled: bool = True):
    override_settings(
        Settings(
            ...
            oidc_issuer_url="https://idp.example/realms/insurancegovernor",
            oidc_audience="insurancegovernor-sidecar",
            oidc_viewer_roles="viewer",
            oidc_claims_admin_roles="claims-admin",
...
def test_extract_roles_keycloak_and_okta_claims() -> None:
    roles = _extract_roles(
        {
            "realm_access": {"roles": ["claims-admin", "offline_access"]},
            "groups": ["Everyone"],
        }
    )
    assert "claims-admin" in roles
...
def test_oidc_claims_admin_can_clear_diagnostic_mode(spine_db, monkeypatch) -> None:
    ...
    def fake_validate(token: str) -> AuthContext:
        return AuthContext(method="oidc", subject="admin@corp", roles=frozenset({"claims-admin"}))
