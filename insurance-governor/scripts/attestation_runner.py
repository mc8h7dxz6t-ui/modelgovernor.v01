#!/usr/bin/env python3
"""Insurance Governor pilot/cluster attestation — produces JSON artifact for data room."""
...
def run_attestation() -> dict[str, Any]:
    token = os.environ.get("IG_INTERNAL_TOKENS", "dev-ig-spine-token-change-me")
    sidecar = _env("IG_SIDECAR_URL", "http://localhost:8101")
    gateway = _env("IG_GATEWAY_URL", "http://localhost:8100")
    claim_gate = _env("IG_CLAIM_GATE_URL", "http://localhost:8103")
    cluster = os.environ.get("IG_CLUSTER_ATTESTATION", "false").lower() == "true"
    environment = os.environ.get("IG_ATTESTATION_ENV", "compose-local" if not cluster else "customer-vpc-staging")
