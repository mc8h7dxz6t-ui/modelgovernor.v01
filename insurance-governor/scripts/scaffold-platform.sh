#!/usr/bin/env bash
# Scaffold a new plug-and-play Insurance Governor platform
set -euo pipefail
...
WORKDIR /app/insurance-governor
ENV PYTHONDONTWRITEBYTECODE=1 PYTHONUNBUFFERED=1 PYTHONPATH=/app/insurance-governor
COPY insurance-governor/platforms/requirements.txt /tmp/requirements.txt
RUN pip install --no-cache-dir -r /tmp/requirements.txt
COPY insurance-governor/platforms/common ./platforms/common
COPY insurance-governor/platforms/$NAME ./platforms/$NAME
EXPOSE $PORT
...
services:
  $NAME:
    build:
      context: ../..
      dockerfile: insurance-governor/platforms/$NAME/Dockerfile
    environment:
      IG_SPINE_ENABLED: "false"
    ports:
      - "$PORT:$PORT"
EOF
