...
def _run_audits(session) -> bool:
    ...
    from app.claim_ops import ClaimOpsInvariantError, assert_claim_ops_invariants
    ...
    try:
        ...
        return True
    except ClaimOpsInvariantError as exc:
        get_counters().increment("claim_audit_violation_total")
        enter_diagnostic_mode(component="reconciler", reason=str(exc))
        logger.critical("ig spine audit failed: %s", exc)
        return False
