"""Insurance claim-plane invariant probes — zero error budget."""
...
class ClaimOpsInvariantError(Exception):
    pass


def assert_claim_ops_invariants(session: Session) -> dict[str, int]:
    violations: dict[str, int] = {}

    negative = session.execute(text("SELECT COUNT(*) FROM reserve_ledgers WHERE balance < 0")).scalar_one()
    ...
    committed_without_crystal = session.execute(
        text(
            """
            SELECT COUNT(*) FROM claim_escrow_ledger e
    ...
    high_risk_auto_expired = session.execute(
        text(
            ...
            JOIN coverage_policy_registry p ON c.policy_id = p.policy_id
    ...
    duplicate_commits = session.execute(
        text(
            """
            SELECT COUNT(*) FROM (
                SELECT operation_id FROM claim_events
    ...
    if sum(violations.values()):
        raise ClaimOpsInvariantError(f"claim ops violations: {violations}")
    return violations
