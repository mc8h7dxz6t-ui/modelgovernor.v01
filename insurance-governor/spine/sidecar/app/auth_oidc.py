"""OAuth2/OIDC JWT validation for enterprise /internal/* RBAC."""
...
class AuthContext:
    ...
    def is_claims_admin(self) -> bool:
        settings = get_settings()
        if self.method == "internal_token" and settings.oidc_internal_token_is_admin:
            return True
        return self.has_role(*settings.oidc_claims_admin_roles_list())

    def is_viewer(self) -> bool:
        if self.is_claims_admin():
            return True
        settings = get_settings()
...
def _validate_internal_token(x_internal_token: str | None) -> AuthContext | None:
    settings = get_settings()
    allowed = {t.strip() for t in settings.ig_internal_tokens.split(",") if t.strip()}
    if x_internal_token and x_internal_token in allowed:
        return AuthContext(
            method="internal_token",
            subject="internal-token",
            roles=frozenset({"claims-admin", "viewer"}),
        )
    return None
...
async def require_claims_admin(
    ...
    if not ctx.is_claims_admin():
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="claims admin role required")
    return ctx
