"""Platform invariant counters — zero error budget signals for Insurance Governor wedges."""
...
def render_prometheus_text(platform: str) -> str:
    ...
    lines = [
        "# HELP ig_platform_invariant_events_total Platform invariant counters.",
        "# TYPE ig_platform_invariant_events_total counter",
    ]
    for name, value in sorted(counters.snapshot().items()):
        if value:
            lines.append(
                f'ig_platform_invariant_events_total{{platform="{platform}",event="{name}"}} {value}'
            )
    return "\n".join(lines) + "\n"
