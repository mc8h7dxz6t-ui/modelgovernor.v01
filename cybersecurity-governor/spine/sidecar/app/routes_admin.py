...
def reconstruct_crystal(crystal_id: str, _: AuthContext = Depends(require_internal_auth)) -> dict:
    with get_db_session() as session:
        ...
        events = session.execute(
            text("SELECT event_type, metadata, recorded_at FROM security_events WHERE crystal_id = :c ORDER BY event_id"),
            {"c": crystal_id},
        ).mappings().all()
        escrow = session.execute(
            text("SELECT * FROM security_escrow_ledger WHERE crystal_id = :c"),
            {"c": crystal_id},
        ).mappings().first()
        return {
...
@router.post("/security/anchor-head")
def anchor_head(ctx: AuthContext = Depends(require_security_admin)) -> dict:
    with get_db_session() as session:
        try:
            result = anchor_verified_chain_head(session, source="api")
        except ValueError as exc:
            raise HTTPException(status_code=422, detail=str(exc)) from exc
        record_admin_action(session, ctx=ctx, action="ANCHOR_HEAD", resource="security_chain", details=result)
        return result
...
def diagnostic_clear(ctx: AuthContext = Depends(require_security_admin)) -> dict:
...
def recent_events(limit: int = 20, _: AuthContext = Depends(require_internal_auth)) -> list:
    with get_db_session() as session:
        rows = session.execute(
            text(
                "SELECT event_id, operation_id, event_type, recorded_at FROM security_events ORDER BY event_id DESC LIMIT :l"
            ),
            {"l": limit},
        ).mappings().all()
