...
from .security_events import append_security_event
...
from spine_core.commit_helpers import money_param, ts_param as _ts_param, utcnow as _utcnow
from spine_core.commit_mesh import mesh_block_reason


def _money_param(value: Decimal) -> str | Decimal:
    return money_param(value, quantize_money)
    ...
    *,
    ...
    if policy_id:
        pol = session.execute(
            text(
                """
                SELECT commit_horizon_ms, max_budget_per_commit, enabled
                FROM security_policy_registry WHERE policy_id = :pid
                """
            ),
            {"pid": policy_id},
        ).mappings().first()
        ...
        max_res = quantize_money(pol["max_budget_per_commit"])
        if reserved_budget > max_res:
    ...
    reserved = quantize_money(reserved_budget)
    if reserved > 0:
        lock_clause = " FOR UPDATE" if session.bind.dialect.name == "postgresql" else ""
        bal = session.execute(
            text(
                f"""
                SELECT balance, active FROM security_budget_ledgers
    ...
    session.execute(
        text(
            """
            INSERT INTO security_escrow_ledger (
                operation_id, crystal_id, account_id, platform,
                reserved_budget, status, expires_at
            ) VALUES (
...
def commit_operation(
    ...
    *,
    ...
    row = session.execute(
        text(
            ...
            SELECT c.crystal_id, c.platform, c.operation_id, c.risk_tier, c.facets,
                   ...
                   e.account_id, e.status, e.reserved_budget
    ...
    committed = quantize_money(committed_budget)
    reserved = quantize_money(row["reserved_budget"])
    ...
    session.execute(
        text(
            ...
            SET status = 'COMMITTED', committed_budget = :ce, committed_at = :now
