#!/usr/bin/env bash
# Cybersecurity Governor pilot attestation — end-to-end spine + platform demo
set -euo pipefail
...
curl -sf -X POST "$GATEWAY/governed/commit" \
  ...
  -d '{"platform":"claim_gate","operation_id":"pilot-claim-1","facets":{"claim_id":"pilot-claim-1","payout_amount":"100.00"},"policy_id":"claim-high-us","reserved_budget":"100","committed_budget":"100","outcome":"paid"}' \
  >/dev/null
...
cd "$ROOT"
python3 cybersecurity-governor/scripts/attestation_runner.py || true
