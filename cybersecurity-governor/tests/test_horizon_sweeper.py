...
from support.reconciler_loader import load_reconciler_module
    ...
    session.execute(
        text(
            ...
            INSERT INTO governance_crystals (
                ...
                :cid, 'egress_govern', :op, :tier, :facets, 'fp', 'ch', :horizon
            )
            """
        ),
        {
            ...
            "op": operation_id,
            "tier": risk_tier,
            "facets": json.dumps({"flow_id": operation_id, "destination_host": "api.openai.com", "egress_decision": "ALLOWED"}),
    ...
    session.execute(
        text(
            ...
            INSERT INTO security_escrow_ledger (
                ...
                :op, :cid, 'tenant-default', 'egress_govern', 10.0, 'CRYSTALLIZED', :horizon
            )
            """
        ),
...
def test_high_risk_horizon_strands(spine_db):
    ...
    with factory() as session:
        ...
        status = session.execute(
            text("SELECT status FROM security_escrow_ledger WHERE crystal_id = 'c-strand'")
        ).scalar_one()
...
def test_in_flight_horizon_strands(spine_db):
    ...
    with factory() as session:
        ...
        session.execute(
            text("UPDATE security_escrow_ledger SET status = 'IN_FLIGHT' WHERE crystal_id = 'c-inflight'")
        )
        session.commit()
